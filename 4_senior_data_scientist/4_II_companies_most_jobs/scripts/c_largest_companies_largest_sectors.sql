...
4_II.c. Use the previous SQL script in order to retrieve the top 5 companies of the top 6 sectors.

Notes:
1. Keep everything in '2_sectors.sql' as it is, but remove the main query.
2. Create 6 SELECT statements (one for each sector).
3. For each SELECT statement:
i. Extract a different sector (sector_name = '...').
ii. Sort the results in descending order by the total number of Senior Data Scientist job postings each company offers (senior_DS_companies).
iii. Retrieve only the top 5 companies of that sector (Limit 5).
4. Use UNION to combine the result sets into a single result set.
